...
with app.app_context():
    ...
    # 1. Сначала ищем или подготавливаем роль
    ...
    # 2. Проверяем админа
    if not db.session.execute(db.select(User).where(User.login == "admin")).scalar():
        admin_user = User(
            login="admin",
            password_hash=generate_password_hash("Admin123!"),
            first_name="Система",
            last_name="Администратор",
            role=admin_role # <--- ПИШЕМ 'role' (ОБЪЕКТ), А НЕ 'role_id'
        )
...
def admin_count():
    count = db.session.execute(
        ...
        .join(User.role) # Делаем JOIN с таблицей Role
...
@app.before_request
def log_visit():
    # Получаем путь, на который зашел пользователь
    path = request.path
    
    user_id = current_user.id if current_user.is_authenticated else None
    
    # Игнорируем запросы к статике (картинки, стили), чтобы не забивать базу
...
def user_delete(id):
    ...
    if user.role and user.role.name == "Администратор":
        if admin_count() <= 1:
            flash("Нельзя удалять последнего Администратора!", "warning")
            return redirect(url_for("index"))
    try:
